 ...
 * description: blowfish 加密 工具类
 */
public class BlowfishUtils {


    private BlowfishCBC mBlowfish;
    private static final Random m_rndGen = new SecureRandom();
    ...
    /**
     * Creates a new Blowfish object using the specified key (oversize password will be cut).
     *
     * @param password the password (treated as a real unicode array)
     */
    public BlowfishUtils(String password) {
        setKey(password);
    }
     ...
     * @return encrypted string in bin-hex format
     */
    ...
    private String encStr(String sPlainText, long lNewCBCIV) {
        ...
        synchronized (mBlowfish) {
            // create the encryptor
            mBlowfish.setCBCIV(lNewCBCIV);

            // encrypt the buffer
            mBlowfish.encrypt(buf);
        }

        // return the bin-hex string
        byte[] newCBCIV = new byte[BlowfishCBC.BLOCK_SIZE];
        longToByteArray(lNewCBCIV, newCBCIV, 0);

        return bytesToBinHex(newCBCIV, 0, BlowfishCBC.BLOCK_SIZE) + bytesToBinHex(buf, 0, buf.length);
    }


    /**
     * decrypts a hex-bin string (handling is case sensitive)
     *
     * @param sCipherText hex-bin string to decrypt
    ...
    public String decryptString(String sCipherText) {
        ...
        if (nLen < BlowfishECB.BLOCK_SIZE)
            return null;

        // get the CBC IV
        byte[] cbciv = new byte[BlowfishCBC.BLOCK_SIZE];
        int nNumOfBytes = binHexToBytes(sCipherText, cbciv, 0, 0, BlowfishCBC.BLOCK_SIZE);
        if (nNumOfBytes < BlowfishCBC.BLOCK_SIZE)
            return null;

        // something left to decrypt?
        nLen -= BlowfishCBC.BLOCK_SIZE;
        if (nLen == 0) {
            return "";
        }
        ...
        nNumOfBytes = binHexToBytes(sCipherText, buf, BlowfishCBC.BLOCK_SIZE * 2, 0, nLen);

        // we cannot accept broken bin-hex sequences due to padding
        // and decryption
        if (nNumOfBytes < nLen) {
            return null;
        }
        ...
        synchronized (mBlowfish) {
            // (got it)
            mBlowfish.setCBCIV(cbciv);

            // decrypt the buffer
            mBlowfish.decrypt(buf);
        }
    ...
    private static class BlowfishECB {
         ...
         */
        public final static int MAX_KEY_LENGTH = 56;
         ...
         * default constructor
         *
         * @param bfkey key material, up to MAX_KEY_LENGTH bytes
         */
    ...
    private static class BlowfishCBC extends BlowfishECB {
         ...
         *
         * @param bfkey     key material, up to MAX_KEY_LENGTH bytes
         * @param initCBCIV the CBC IV (array with min. BLOCK_SIZE bytes)
         */
    ...
    private static String bytesToBinHex(byte[] data, int nStartPos, int nNumOfBytes) {
        ...
        builder.setLength(nNumOfBytes << 1);

        int nPos = 0;
        for (int nI = 0; nI < nNumOfBytes; nI++) {
            builder.setCharAt(nPos++, HEXTAB[(data[nI + nStartPos] >> 4) & 0x0f]);
            builder.setCharAt(nPos++, HEXTAB[data[nI + nStartPos] & 0x0f]);
        }
    ...
    /*public static void main(String[] args) {
        String passWordKey = "naPy72C0TDHOf4G"; //秘钥
        BlowfishUtils blowFish = new BlowfishUtils(passWordKey); //根据加密key初始化
        //passWordKey = blowFish.encryptString(resultPWD); //加密
        //System.out.println(passWordKey);//解密
        //System.out.println("加密出来后的结果==="+blowFish.encrypt("admin"));
        System.out.println("出来后的结果===" + blowFish.decrypt("000000000000000385b443a58cb8a40f"));
    }*/
}
